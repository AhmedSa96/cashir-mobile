...
abstract class $AuthStatusCopyWith<$Res> {
  ...
  @useResult
  $Res call({bool isLoggedIn, String? token, User? user});
...
class _$AuthStatusImpl implements _AuthStatus {
  const _$AuthStatusImpl({required this.isLoggedIn, this.token, this.user});
  ...
  @override
  String toString() {
    return 'AuthStatus(isLoggedIn: $isLoggedIn, token: $token, user: $user)';
  }
  ...
  @JsonKey(ignore: true)
  @override
  int get hashCode => Object.hash(runtimeType, isLoggedIn, token, user);
...
abstract class _AuthStatus implements AuthStatus {
  const factory _AuthStatus(
      ...
      final String? token,
      final User? user}) = _$AuthStatusImpl;
